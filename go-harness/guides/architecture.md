...
> 通用知识以 skill senior-go-engineer（单体 / 微服务判定、分层与包设计的取舍框架）；本文是本项目落定后的结构，不再讨论取舍 为准；本文只写本项目的约定与落地位置，不重复那里的内容。
...
- 用结构体承载配置，不用散落全局变量
- 配置从外置 YAML 文件加载（config/env/env.yml）：进程启动时一次性读取并校验，找不到或缺关键项 fail-fast；不再编译进二进制
- 运行环境由配置文件内的 env 字段决定（dev/test/prod），不依赖文件名或命令行参数；各环境一机一份、严格隔离
- 敏感配置（密钥、密码、证书）不进版本控制；密钥与证书走外部注入 / 文件挂载，绝不入库