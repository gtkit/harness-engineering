...
7. **量化自检**：写完自问"senior 会不会觉得过度复杂？200 行能否压到 50 行？"——答"是"就重写；单次使用的代码不写抽象 / 配置项 / 扩展点。
...
- 依赖使用最新稳定版，不用 RC/Beta；JSON 属于例外，必须使用 `github.com/gtkit/json` 或 `github.com/gtkit/json/v2`
...
- 后台 worker / 定时任务 / 队列消费 → 读 `.harness/guides/workers-and-scheduling.md`
- 写扩展包 → 读 `.harness/guides/pkg-design.md`
- 测试 / 回归 / 验证 → 读 `.harness/guides/testing-and-validation.md`
...
- **代码复用**：优先复用已有 helper、service、repository、middleware、validator 和工具函数；新增抽象必须至少解决真实重复或隔离复杂度
- **架构清晰**：代码必须放在职责匹配的层；禁止为了方便把业务逻辑塞进 handler、repository、model 或中间件
- **分层合理**：上层只编排，下层只封装细节；跨层调用、循环依赖、业务层依赖 Gin、数据层反向依赖业务层均视为违规
- **健壮稳定**：错误路径、nil、边界输入、超时、取消、资源释放、并发安全、外部依赖失败必须有明确处理
