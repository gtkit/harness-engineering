...
- `COVERAGE_MIN` 默认值从 80 改为 0。发版流程不是发现覆盖率不足的合适时机——那时改代码已经太晚，而实测存量库覆盖率两极分化（抽样 5 个中 2 个为 0.0%），80 的默认值会让这些包直接发不了版。要卡的包在自己的 Makefile 里设具体数字。
...
### Changed
- setup 跳过已存在文件时先比对内容，把"跳过"细分成两种结果：与本版本模板一致的显示"已存在且与本版本一致"，内容不同的显示"已存在且内容与本版本模板不同，保留未动"（脚本只能判断"不同"，不能判断方向——差异既可能是本地落后，也可能是本地有意定制，提示里要求逐个 diff 确认后再决定是否强刷），并在安装结束时汇总有差异的项目文件与 guides 清单、附上强制刷新命令。此前跨版本重装时，落后的 `CLAUDE.md` / `AGENTS.md` 与 guides 只打印一条"已存在，跳过"，安装看起来成功而规则实际仍停留在旧版本，升级三个版本也察觉不到。汇总里同时提示刷新 `CLAUDE.md` / `AGENTS.md` 是整文件覆盖，若这两个文件里有 openspec-auto 等其他工具写入的托管块，覆盖后需重新执行对应安装器补回。`scripts/install-harness.sh` 与 `scripts/install-harness.ps1` 两端行为一致，比对分别用 `cmp -s` 与 `Get-FileHash -Algorithm SHA256`；默认行为不变，仍然不覆盖任何已存在文件。
