...
9. 多解陈列：指令存在多种合理解释时，并列呈现给用户选择，不默默择一实现。
10. 反推更简方案：发现比用户原方案更简单的做法（能用 `std::span` 不用自写容器、能用 `std::jthread` 不用裸 `pthread`），主动提出并说明权衡。
11. 量化自检：写完自问"senior 会不会觉得过度复杂？200 行能否压到 50 行？"；单次使用的代码不写抽象 / 配置项 / 扩展点；模板元编程与宏必须有不可替代理由。
...
每一行 diff 必须能追溯到用户的本次请求。C++ 代码的副作用半径大，改动尤其要克制。

- **不顺手改**：相邻无关代码、注释、格式、命名、include 顺序一律不动
- **不重构未坏的代码**：你偏好的写法不是改动理由，匹配既有风格
- **只清自己的孤儿**：本次改动产生的未引用 include / 变量 / 函数 / 类成员必须清理；既有死代码发现了**提一下，别删**
- **ABI 保护**：改动不得破坏既有 ABI（导出符号、vtable 布局、结构体内存布局、默认参数顺序）；确需变更时必须告知用户并更新版本号
- **构建范围最小化**：不新增 CMake target、不改 compile flag、不改 toolchain，除非用户明确要求
...
| 模糊指令 | 可验证目标 |
|---------|----------|
| "加个校验" | 写非法输入单测（GoogleTest/Catch2）→ 让它通过 |
| "修这个 bug" | 写复现用例（必要时加 ASan/TSan/UBSan 运行）→ 让它通过 |
| "重构 X" | 改前测试全绿 + Sanitizer 无告警 → 改后保持 + benchmark 不退化 |
| "优化性能" | 先写 benchmark 基线 → 改后对比 p50/p99 → 达到目标阈值 |
| "让它能跑" | 不可验证，退回用户澄清成功标准（吞吐？延迟？资源？） |

**多步任务先列计划：**

    1. [步骤] → verify: [可观察的检查，如 ctest / perf / valgrind 输出]
    2. [步骤] → verify: [可观察的检查]
