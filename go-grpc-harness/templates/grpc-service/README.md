...
- 分层骨架（bootstrap/runtime/module/repository/models/platform/pkg）+ 架构传感器（`scripts/check-architecture.sh`，纳入 `make check`）
- gRPC 拦截器链：recovery → 限流（golimit，配置可关）→ protovalidate
...
- 优雅关闭（SIGTERM drain；serve 错误经 channel 传播，不在 goroutine 里 Fatal）

## 模板未带（按需自加，guides 有对应规范）

健康探针（grpc.health.v1）、鉴权/mTLS（模板默认内网明文）、metrics、request_id、Dockerfile、Redis（引入时用 gtkit/redisx）、grpc-gateway 接线（pb.gw.go 已生成未接）。
